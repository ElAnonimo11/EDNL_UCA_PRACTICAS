...
vector<tCoste> DijkstraInv(const GrafoP<tCoste>& G,
                        ...
                        vector<typename GrafoP<tCoste>::vertice>& P)
// Calculo de el coste minimo de todo el resto de nodos a el destino
{
    using vertice = typename GrafoP<tCoste>::vertice;
    // Voy a hacerlo haciendo la matriz traspuesta
    const size_t n = G.numVert();
    matriz<tCoste> matriz_costes(n),matriz_trasp(n);
    for(vertice v = 0; v < n ; ++v)
        matriz_costes[v] = G[v];

    // Hacemos la traspuesta a la matriz
    for(vertice i = 0; i < n ; ++i)
        for(vertice j = 0; j < n ; ++j)
            matriz_trasp[j][i] = matriz_costes[i][j];
    GrafoP<tCoste> aux(n);
    for(vertice i = 0; i < n ; ++i)
        aux[i] = matriz_trasp[i];

    return Dijkstra(aux, destino, P);
}
